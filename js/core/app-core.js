...
// elivandro daqui para baixo (função que lê do banco)
async function loadPropostasFromSupabase(){
  if(!window.sbClient){
    console.warn('Supabase não conectado para leitura.');
    return;
  }
  ...
  const { data, error } = await window.sbClient
    .from('propostas')
    .select('dados_json, updated_at')
    .eq('user_id', user.id)  // ← NOVO: só carrega propostas do usuário
  ...
  if(!data || !data.length){
    console.log('Nenhuma proposta encontrada no Supabase.');
    return;
  }

  var lista = data
    .map(function(row){ return row.dados_json; })
  ...
  if(lista.length){
    ...
    rDash();
    rProps();
    try{ rAnalise(); }catch(e){}
    try{ rRegistro(); }catch(e){}
    try{ rT(); }catch(e){}
    try{ rS(); }catch(e){}
    try{ refCat(); }catch(e){}
    try{ rEsc(); }catch(e){}
    console.log('Propostas carregadas do Supabase:', lista.length);
  }
}
// elivandro daqui para cima (função que lê do banco)
...
async function syncProposalToSupabase(sn){
  if(!window.sbClient){
  ...
  var payload = {
    app_id: String(sn.id || ''),
    numero_proposta: String(sn.num || Q('pNum').value || '').trim(),
    titulo: String(sn.tit || Q('pTit').value || '').trim(),
    cliente: String(sn.cli || Q('pCli').value || '').trim(),
    valor_total: Number(sn.totalGeral || sn.total || 0) || 0,
    fase: String(sn.fas || Q('pFas').value || '').trim(),
    dados_json: sn,
    user_id: user.id,  // ← NOVO: vincula proposta ao usuário
    updated_at: new Date().toISOString()
  };

  const { data: existente, error: erroBusca } = await window.sbClient
    .from('propostas')
    .select('id, app_id')
...
// elivandro nova função saveP() daqui para baixo
async function saveP(){
  ...
  try{
    await syncProposalToSupabase(sn);
    try{showActionBar(sn);}catch(e){}
    toast('✔ Proposta salva local + nuvem!','ok');
  }catch(e){
    console.error('Erro ao salvar no Supabase:', e);
    try{showActionBar(sn);}catch(err){}
    toast('⚠ Salvou local, mas falhou na nuvem.','err');
  }
}
// elivandro nova função saveP() daqui para cima
...
// INIT Quando o sistema abrir: 
//  1- ele continua carregando local como hoje
//  2- depois busca no Supabase
//  3- se encontrar propostas, joga elas em props
//  4- salva local de novo
//  5- redesenha as telas principais
document.addEventListener('DOMContentLoaded',async function(){
  ...
  try{
    await loadPropostasFromSupabase();
  }catch(e){
    console.error('Falha ao sincronizar propostas na abertura:', e);
  }
});
// ══════════════════════════════════════════════
// ANÁLISE FINANCEIRA
